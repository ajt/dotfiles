...
render() {
  ...
  # Keep alive until the main pane disappears. `sleep & wait` so the WINCH trap
  # can interrupt the wait and redraw without killing the loop.
  while tmux list-panes -F '#{pane_id}' 2>/dev/null | grep -qx "$main"; do
    sleep 5 &
    wait "$!" 2>/dev/null
  done
}
...
show() {
  ...
  bp=$(tmux split-window -vb -l 3 -t "$main" -P -F '#{pane_id}' "$SELF render $main") || return 1
